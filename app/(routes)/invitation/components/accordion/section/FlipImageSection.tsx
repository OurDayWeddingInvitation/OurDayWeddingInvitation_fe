  ...
  useEffect(() => {
    const clickOutside = (e: MouseEvent) => {
      if (
        ...
        !titlePickerRef.current.contains(e.target as Node)
      ) {
        setTitlePickerOpen(false);
      }
      if (
        contentPickerRef.current &&
        !contentPickerRef.current.contains(e.target as Node)
      ) {
    ...
    <div>
        ...
        <div className="flex flex-wrap items-center pb-3">
          ...
          <div className="flex gap-2 flex-1 items-end">
            <textarea
              ...
              defaultValue={
                "스크롤을 내리면 우리 두 사람의 특별한 이야기가 펼쳐져요."
              }
              onChange={(e) => {}}
            />
            <div>
      ...
      <div>
        <div className="flex gap-3 pt-[17px]">
          <h3>플립북 영상 올리기</h3>
          <button className="text-[10px] border px-2 py-1 border-[#D4C6B7] rounded-sm">
            업로드
          </button>
        </div>
        <p className="text-[12px] text-[#CACACA] pb-[17px] pt-2">
          영상을 올리면 자동으로 연속 사진으로 변환됩니다.(5초 이하의 짧은
          영상만 가능)
        </p>
        ...
        <div className="border-[#D9D9D9] border rounded-[10px] w-full h-[291px] p-4 mb-5">
          ...
          />
          <label
            htmlFor="flipbookInput"
            className="w-[124px] h-[124px] cursor-pointer"
          >
            <Image src={ImageAddBtnIcon} alt="추가" />
          </label>
        </div>
        <p className="text-[12px] pb-[30px] text-[#CACACA]">
          영상이 없을 경우 원하는 연속 사진을 직접 올려 플립북을 만들 수 있어요.
        </p>
        ...
        <div className="text-[#CACACA] text-[12px] mt-[30px]">
          ...
          <p>
            짧은 영상을 여러 장의 연속 사진으로 변환해, 스크롤을 내릴 때 사진이
            움직이는 것처럼 보이게 하는 기능입니다.
          </p>
